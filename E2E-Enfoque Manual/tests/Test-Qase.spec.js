// Pruebas de Regresion E2E con Playwright
...
test.describe.serial('Regresión E2E - Sprint 1', () => {
  ...
  test('Registro de asociado exitoso @QaseID=22', async ({ page }) => {
    ...
    await page.getByRole('textbox').nth(5).fill('CURP123456789');
    ...
    // Ajusta este texto al mensaje real que muestre tu app
  ...
  test('Registro de asociado negativo por CURP vacía @QaseID=22', async ({ page }) => {
    ...
    // Ajusta al mensaje real de validación
  ...
  test('Registro de historial de Padres @QaseID=31', async ({ page }) => {
    ...
    // Ajusta al mensaje real que muestre tu app
    await expect(page).toHaveURL(/.*asociados/);
  });

  test('EBCDC-31 - Registro de historial de padres negativo por campo vacío @QaseID=31', async ({ page }) => {
    await login(page);
    ...
    // Dejar un campo obligatorio vacío a propósito
